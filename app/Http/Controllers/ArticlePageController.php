...
class ArticlePageController extends Controller
{
    ...
    public function show(string $college, string $slug): \Illuminate\Http\RedirectResponse|\Illuminate\View\View
    {
        ...
        // Redirect back if article not found OR does not match college
        if (!$article || ($article->college_slug && $article->college_slug !== $college)) {
        ...
        // Get related articles (same type, same college or all colleges, excluding current article)
        $relatedArticles = Article::where('type', $article->type)
            ->whereIn('college_slug', [$college, null])
            ->where('id', '!=', $article->id)
            ->whereNotNull('published_at')
        ...
        // Get recent articles (same type, same college or all colleges, excluding current article)
        $recentArticles = Article::where('type', $article->type)
            ->whereIn('college_slug', [$college, null])
            ->where('id', '!=', $article->id)
            ->whereNotNull('published_at')
            ->orderBy('published_at', 'desc')
        ...
        // Create article object for view compatibility
        // ... (existing logic)
