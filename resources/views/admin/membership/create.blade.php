...
@section('title', 'Add Membership Record')
    ...
    <div class="row mb-4">
        <div class="col-12" style="background: #e1f5e9; padding: 2rem; border-radius: 15px; border-left: 5px solid #0d6e42;">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="h3 mb-1 fw-800" style="color: #0d2818;">Add Membership Record</h1>
                    <p class="text-muted mb-0">
                        {{ isset($departmentContext) ? 'Create a new professional affiliation or membership for this department.' : 'Create a new professional affiliation or membership for a college or department.' }}
                    </p>
                </div>
                <a href="{{ isset($departmentContext) ? route('admin.colleges.show-department', ['college' => $collegeSlug, 'department' => $departmentContext, 'section' => 'membership']) : ($fromCollegeSection ? route('admin.colleges.show', ['college' => $collegeSlug, 'section' => 'membership']) : route('admin.colleges.index')) }}" class="btn btn-outline-secondary btn-sm rounded-pill px-3">
                    Back to {{ isset($departmentContext) ? 'Department' : ($collegeSlug ? 'College' : 'Dashboard') }}
                </a>
            </div>
        </div>
    </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="college_slug" id="college_slug" class="form-select rounded-3 @error('college_slug') is-invalid @enderror" required>
                            ...
                            @foreach($colleges as $slug => $name)
                                <option value="{{ $slug }}" {{ (old('college_slug') == $slug || $collegeSlug == $slug) ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                        @error('college_slug')
                    ...
                    @if(isset($departmentContext))
                        <div class="col-md-6">
                            ...
                            <small class="text-muted">This membership will be saved under this department.</small>
                        </div>
                    @else
                        <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="organization" id="organization" class="form-control rounded-3 @error('organization') is-invalid @enderror" value="{{ old('organization') }}" placeholder="e.g., PSITE, IEEE" required>
                        @error('organization')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="membership_type" id="membership_type" class="form-control rounded-3 @error('membership_type') is-invalid @enderror" value="{{ old('membership_type') }}" placeholder="e.g., Institutional Member, Platinum Partner" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <textarea name="description" id="description" class="form-control rounded-3 @error('description') is-invalid @enderror" rows="2" placeholder="Optional extra details about the membership.">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="valid_until" id="valid_until" class="form-control rounded-3 @error('valid_until') is-invalid @enderror" value="{{ old('valid_until') }}">
                        @error('valid_until')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 mt-4 pt-3 border-top">
                        <button type="submit" class="btn btn-admin-primary px-5 rounded-pill shadow-sm">Save Membership</button>
                        <a href="{{ isset($departmentContext) ? route('admin.colleges.show-department', ['college' => $collegeSlug, 'department' => $departmentContext, 'section' => 'membership']) : ($fromCollegeSection ? route('admin.colleges.show', ['college' => $collegeSlug, 'section' => 'membership']) : route('admin.colleges.index')) }}" class="btn btn-light px-4 rounded-pill ms-2">Cancel</a>
                    </div>
                </div>
            </form>
        </div>
