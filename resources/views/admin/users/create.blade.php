...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="admin-page-title mb-0">Add User</h1>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="role" id="role" class="form-select @error('role') is-invalid @enderror" required>
                            ...
                            @if (auth()->user()?->isSuperAdmin())
                                <option value="superadmin" {{ old('role') === 'superadmin' ? 'selected' : '' }}>Superadmin (all colleges)</option>
                            @endif
                        </select>
                    ...
                    <div class="col-md-6" id="college_slug-wrap">
                        ...
                        <select name="college_slug" id="college_slug" class="form-select @error('college_slug') is-invalid @enderror">
                            <option value="">— Select college (Superadmin only) —</option>
                            @foreach ($colleges as $slug => $name)
                                <option value="{{ $slug }}" {{ old('college_slug') === $slug ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                        <small class="text-muted">Editor and admin can only access their assigned college. Superadmin has all colleges.</small>
                        @error('college_slug')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
    ...
    <script>
        ...
        const oldOrg = "{{ old('organization_id') }}";
        ...
        function updateOrganizations() {
            ...
            if (selectedCollege && organizationsByCollege[selectedCollege]) {
                const orgs = organizationsByCollege[selectedCollege].filter(org => {
                    // Include college-wide organizations (department_id null) 
                    // and ones that match the selected department if one is selected.
                    if (selectedDeptId) {
