...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="admin-page-title mb-0">Add Faculty</h1>
        @if (!empty($fromCollegeSection) && !empty($collegeSlug) && !empty($returnDepartment))
            <a href="{{ route('admin.colleges.show-department', ['college' => $collegeSlug, 'department' => $returnDepartment, 'section' => 'faculty']) }}" class="btn btn-outline-secondary">Back to Department</a>
        @elseif (!empty($fromCollegeSection) && !empty($collegeSlug))
            <a href="{{ route('admin.colleges.show', ['college' => $collegeSlug, 'section' => 'faculty']) }}" class="btn btn-outline-secondary">Back to {{ \App\Http\Controllers\Admin\CollegeController::getColleges()[$collegeSlug] ?? $collegeSlug }}</a>
        @else
                        ...
                        <div class="row g-4">
                            <div class="col-md-8">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="position" id="position" class="form-control form-control-sm @error('position') is-invalid @enderror" value="{{ old('position') }}" placeholder="e.g. Associate Professor">
                                    @error('position')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="row g-3 mb-3">
                                    ...
                                    @elseif(!empty($collegeSlug))
                                        <input type="hidden" name="college_slug" id="college_slug" value="{{ $collegeSlug }}">
                                    @endif
                                        ...
                                        <select name="department" id="department" class="form-select form-select-sm @error('department') is-invalid @enderror">
                                            <option value="">Select Department</option>
                                                @foreach ($departments as $dept)
                                                    <option value="{{ $dept->name }}" data-college="{{ $dept->college_slug }}" {{ old('department', $selectedDepartment ?? '') === $dept->name ? 'selected' : '' }}>
                                                    {{ $dept->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="mt-4 pt-2 border-top">
                                    <button type="submit" class="btn btn-primary btn-sm px-4">Save Faculty</button>
                                    @if (!empty($fromCollegeSection) && !empty($collegeSlug) && !empty($returnDepartment))
                                        <a href="{{ route('admin.colleges.show-department', ['college' => $collegeSlug, 'department' => $returnDepartment, 'section' => 'faculty']) }}" class="btn btn-outline-secondary btn-sm px-4 ms-2">Cancel</a>
                                    @elseif (!empty($fromCollegeSection) && !empty($collegeSlug))
                                        <a href="{{ route('admin.colleges.show', ['college' => $collegeSlug, 'section' => 'faculty']) }}" class="btn btn-outline-secondary btn-sm px-4 ms-2">Cancel</a>
                                    @else
                            ...
                            <div class="col-md-4">
                                <div class="p-3 border rounded bg-light h-100">
                                    ...
                                    <div class="photo-upload-container text-center">
                                        ...
                                        <small class="text-muted d-block mt-2">To fit on the page, please use a 2x2 image.</small>
                                        @error('photo')
