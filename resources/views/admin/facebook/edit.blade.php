...
<div class="admin-card facebook-form-card">
    <div class="facebook-form-header">
        <div>
            <h2 class="facebook-form-title">Edit Facebook Configuration</h2>
            <p class="facebook-form-subtitle">Update the college page mapping, access token, and article capture behavior.</p>
        </div>
        <a href="{{ route('admin.facebook.index') }}" class="btn btn-outline-secondary">Back to list</a>
    </div>
    <form action="{{ route('admin.facebook.update', $facebookConfig) }}" method="POST" class="facebook-form-body">
        @csrf
        @method('PUT')
            ...
            <div class="col-md-6" id="entity_id_field">
                ...
                <select name="entity_id" id="entity_id" class="form-select @error('entity_id') is-invalid @enderror" required>
                    ...
                    @foreach ($colleges as $college)
                        <option value="{{ $college->slug }}" {{ old('entity_id', $facebookConfig->entity_id) === $college->slug ? 'selected' : '' }}>{{ $college->name }}</option>
                    @endforeach
                </select>
