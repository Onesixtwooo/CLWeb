...
<div class="admin-card" style="max-width: 600px;">
    <form action="{{ route('admin.facebook.store') }}" method="POST" class="p-4">
        @csrf
        ...
        @else
            <div class="mb-3">
                ...
                <select name="entity_type" id="entity_type" class="form-select @error('entity_type') is-invalid @enderror" required onchange="updateEntityField()">
                    ...
                    <option value="global" {{ old('entity_type') === 'global' ? 'selected' : '' }}>Global</option>
                    <option value="college" {{ old('entity_type') === 'college' ? 'selected' : '' }}>College</option>
                    <option value="department" {{ old('entity_type') === 'department' ? 'selected' : '' }}>Department</option>
                    <option value="organization" {{ old('entity_type') === 'organization' ? 'selected' : '' }}>Student Organization</option>
                </select>
            ...
            <div class="mb-3" id="entity_id_field" style="display: none;">
                ...
                <select name="entity_id" id="entity_id" class="form-select @error('entity_id') is-invalid @enderror">
                    ...
                    @foreach ($colleges as $college)
                        <option value="{{ $college->slug }}" data-type="college" {{ old('entity_id') === $college->slug ? 'selected' : '' }}>{{ $college->name }}</option>
                    @endforeach
                    @foreach ($departments as $dept)
                        <option value="{{ $dept->id }}" data-type="department" {{ old('entity_id') == $dept->id ? 'selected' : '' }}>{{ $dept->name }}</option>
                    @endforeach
                    @foreach ($organizations as $org)
                        <option value="{{ $org->id }}" data-type="organization" {{ old('entity_id') == $org->id ? 'selected' : '' }}>{{ $org->name }}</option>
                    @endforeach
                </select>
