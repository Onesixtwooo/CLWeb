        ...
        <section id="training" class="py-5">
            <div class="container">
                <div class="college-page">
                    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4" id="training-grid" data-per-page="9">
                        @forelse($trainingItems as $item)
                        <a href="{{ route('college.training.show', ['college' => $collegeSlug, 'slug' => \Illuminate\Support\Str::slug($item->title)]) }}" class="col news-board-card text-decoration-none">
                            <div class="news-board-card-banner">
                                <img src="{{ !empty($item->image) ? \App\Providers\AppServiceProvider::resolveImageUrl($item->image) : $collegeLogoUrl }}" alt="{{ $item->title ?? 'Training' }}" style="{{ empty($item->image) ? 'object-fit: contain; padding: 2rem; background: #f8f9fa;' : '' }}">
                            ...
                            <div class="news-board-card-body p-4 pt-5" style="position: relative; z-index: 1; margin-top: -1px; padding-top: {{ !empty($item->created_at) ? '3.5rem !important' : '2.5rem' }};">
                                <p class="small fw-semibold text-uppercase mb-1" style="color: {{ $headerColor }}; letter-spacing: 0.05em;">Workshop</p>
                        ...
                        <div class="col-12 text-center py-5" style="grid-column: 1 / -1;">
                            <p class="text-muted">No training workshops available at this time.</p>
                        </div>
                        @endforelse
                    </div>

                    @if($trainingItems->count() > 9)
                    <nav class="news-board-pagination" id="training-pagination" aria-label="Training pagination">
                        <ul class="pagination mb-0 flex-wrap justify-content-center gap-1">
                            ...
                            @for($i = 1; $i <= ceil($trainingItems->count() / 9); $i++)
